...
import { find } from "../../../databaseHelper";
...
const MyAvailabilityScreen = ({ navigation }) => {
    ...
    useCallback(() => {
      const fetchSchedules = async () => {
        try {
          ...
          // const docSnap = await getDoc(doc(db, "providerSchedules", userId));
  ...
  const handleAddSchedule = async (newSchedule) => {
    ...
    try {
      await setDoc(doc(db, "providerSchedules", userId), {
  ...
  const handleEditSchedule = async (updatedSchedule) => {
    // Check for overlapping schedules, excluding the current schedule being edited
    const isOverlapping = schedules.some((schedule) => {
  ...
  const handleRemoveSchedule = async (scheduleToRemove) => {
    Alert.alert(
      "Remove Schedule",
          ...
          onPress: async () => {
            try {
              const updatedSchedules = schedules.filter(
                (schedule) => schedule.timestamp !== scheduleToRemove.timestamp
              );
              await setDoc(doc(db, "providerSchedules", userId), {
                schedules: updatedSchedules,
              });
